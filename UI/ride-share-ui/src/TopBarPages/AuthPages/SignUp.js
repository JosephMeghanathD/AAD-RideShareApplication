...
const RoleToggle = ({ role, setRole }) => (
  <div className="relative w-full p-1 bg-slate-700 rounded-xl flex">
    <span
      ...
      style={{
        transform: role === "Driver" ? "translateX(100%)" : "translateX(0)",
      }}
      aria-hidden="true"
    ></span>
    {["Rider", "Driver"].map((r) => (
...
const Signup = () => {
  ...
  const handleSignup = (e) => {
    ...
    const signUpReq = async () => {
      try {
        await axios.post(
          "https://auth-service-1002278726079.us-central1.run.app/api/rs/public/signUp",
          {
            userName,
            password,
            emailId,
        ...
        setErrorMessage(
          error.response?.data?.body?.detail || "An unknown error occurred."
        );
      } finally {
        setLoading(false);
      }
