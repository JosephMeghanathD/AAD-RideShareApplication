    ...
    return (
        <div className="flex items-center justify-between px-4 py-3 bg-slate-800 sm:px-6">
            <div className="flex-1 flex justify-start">
                <PaginationButton onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1 || isLoading}>
                    Previous
                </PaginationButton>
            </div>
            ...
            <div className="flex-1 flex justify-end">
                <PaginationButton onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages || isLoading}>
                    Next
                </PaginationButton>
            </div>
        </div>
...
export const RideTable = ({ rides, handleSort, sortBy, sortOrder, currentPage, totalPages, handlePageChange, error, isLoading }) => {
    ...
    const TableHeader = ({ sortKey, children }) => (
        <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">
            <div className="flex items-center gap-2 cursor-pointer select-none" onClick={() => handleSort(sortKey)}>
                {children}
    ...
    return (
        // Main container for the table, matching the dark card style
                ...
                <table className="min-w-full">
                    <thead className="bg-slate-800">
                        <tr>
                            ...
                            {isLoggedIn && <TableHeader sortKey="postedBy.name">Posted By</TableHeader>}
                            <TableHeader sortKey="postedAt">Posted At</TableHeader>
                            {isLoggedIn && <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Actions</th>}
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-700">
                        {isLoading ? (
                            <tr><td colSpan={colSpan} className="text-center py-10 text-slate-400">Loading rides...</td></tr>
                        ) : rides && rides.length > 0 ? (
                            rides.map((ride) => <RideListRow key={ride.rideId} ride={ride} />)
                        ) : (
                            <tr><td colSpan={colSpan} className="text-center py-10 text-slate-400">No rides found.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
