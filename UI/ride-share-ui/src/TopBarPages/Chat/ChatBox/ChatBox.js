  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.data) {
          const otherUser =
            response.data.userID1.name === localStorage.getItem("currentUser")
              ? response.data.userID2.name
              : response.data.userID1.name;
          setName(otherUser);
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-800">
          ...
          {data.messages &&
            data.messages.map((message) => {
              ...
              return (
                ...
                >
                  <div
                    className={`max-w-md lg:max-w-xl px-4 py-2.5 rounded-2xl ${
                      isCurrentUser
      ...
      >
        ...
        <button
          type="submit"
          className="bg-blue-600 text-white rounded-full p-3 w-12 h-12 flex items-center justify-center hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition-colors"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
