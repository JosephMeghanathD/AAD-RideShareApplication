...
public class Chat {
    ...
    public class ChatDAO extends AbstractCassandraDAO<Chat> {
        public static final PreparedStatement CREATE_STMT = getCqlSession().prepare(
                "CREATE TABLE IF NOT EXISTS " + CHAT_TABLE +
                        " (chatId TEXT PRIMARY KEY, userId1 TEXT, userId2 TEXT, messages TEXT)");
        ...
        public static List<Chat> getAllChats() {
            ...
            BoundStatement boundStatement = getCqlSession().prepare("SELECT * FROM " +
                    CHAT_TABLE).bind();
        ...
        public static List<Chat> getChatsByUserId(String userId) {
            List<Chat> chatList = new ArrayList<>();
            BoundStatement boundStatement = getCqlSession().prepare("SELECT * FROM " +
                    CHAT_TABLE + " WHERE userId1 = ? OR userId2 = ?").bind(userId, userId);
            return mapChats(boundStatement, chatList);
        }
        ...
        public PreparedStatement getUpdateStmt() {
            if (UPDATE_STMT == null) {
                UPDATE_STMT = getCqlSession().prepare("UPDATE " + CHAT_TABLE +
                        " SET messages = ? WHERE chatId = ?");
            }
            return UPDATE_STMT;
        }
        ...
        public PreparedStatement getDeleteStmt() {
            if (DELETE_STMT == null) {
                DELETE_STMT = getCqlSession().prepare("DELETE FROM " + CHAT_TABLE +
                        " WHERE chatId = ?");
            }
            return DELETE_STMT;
        }
        ...
        public PreparedStatement getStmt() {
            if (SELECT_STMT == null) {
                SELECT_STMT = getCqlSession().prepare("SELECT * FROM " + CHAT_TABLE +
                        " WHERE chatId = ?");
            }
            return SELECT_STMT;
        }
